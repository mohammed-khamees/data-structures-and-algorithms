...
const getNames = (arr) => {
  // Solution code here...
  return arr.map((el) => el.name.split("").reverse().join(""));
};
...
const count = (target, input) => {
  // Solution code here...
  let numTargert = input.map((arr) => {
    return arr.filter((num) => {
      return num === target;
    });
  });
  return numTargert.reduce((acc, el) => {
    acc += el.length;
    return acc;
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let filterArr = input.map((arr) =>
    arr.filter((el) => el % 5 === 0 && typeof el !== "string")
  );
  let raisedTwoArrs = filterArr.map((arr) =>
    arr.map((num) => Math.pow(2, num))
  );
  return raisedTwoArrs;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let maleFemaleArr = data.filter(
    (el) => el.gender === "female" || el.gender === "male"
  );
  return maleFemaleArr
    .map((el) => {
      return el.name;
    })
    .join(" and ");
...
let findShortest = (data) => {
  // Solution code here...
  let heightsArr = data.map((el) => el.height);
  let minHeight = Math.min(...heightsArr);
  let shortest = data.find((el) => el.height == minHeight);
