...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((element) => {
    let item = element.indexOf(" ", 5) + 1;
    result.push(element.slice(item));
  });
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  // Solution code here...
  let arr = str.split("");
  arr.splice(-numberOfCharacters, numberOfCharacters);
  return arr.join("");
};
...
const extractVowels = (str) => {
  ...
  strVowels.sort();
  return [noVowels.join(""), strVowels.join("")];
};
