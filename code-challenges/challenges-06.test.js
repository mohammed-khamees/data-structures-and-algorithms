...
const updateNumbers = (obj) => {
  ...
  for (const key in obj) {
    for (let index = 0; index < Object.keys(obj).length; index++) {
      arr.push(`${Object.keys(obj)[index]}: ${Object.values(obj)[index]}`);
    }
  }
  return arr;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  arr.forEach((element) => {
    if (Object.values(element)[0] === character) {
      booleanArr.push(Object.values(element)[2].length);
    }
  });
  if (booleanArr[0]) {
    return true;
...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach((element) => {
    if (Object.entries(element)[0][1] === character) {
      booleanArr.push(Object.entries(element)[2][1].length);
    }
  });
  if (booleanArr[0]) {
    return true;
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach((el) => {
    ...
    if (deceasedSpouses.includes(el.spouse)) {
      total--;
    }
    survivors.push({
      house: el.house,
