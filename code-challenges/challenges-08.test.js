...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  return arr.filter((num) => {
    return forbiddenValues.includes(num) !== true;
  });
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  return arr.filter((el) => minBaseStat < el.baseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  let filteredArr = arr.filter((el) => minBaseStat < el.baseStat);
  return filteredArr.map((el) => el.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  ...
  return newArr.map((el) => (el % 2 === 0 ? "even" : "odd"));
};
