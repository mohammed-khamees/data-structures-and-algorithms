...
function transformToLis(obj) {
  ...
  return keys.map((el, index) => `<li>${el}: ${values[index]}</li>`);
}
...
const addPurchases = (arr) => {
  // Solution code here...
  return arr.reduce((acc, el) => (acc += el.purchasePrice), 0);
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  return arr.reduce((acc, el) => (acc += 1), 0);
};
...
const reversedString = (str) => {
  ...
  return str
    .split("")
    .reduce((acc, char, index) => (acc += str[str.length - index - 1]), "");
};
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  return arr.reduce((acc, el) => {
    if (el.children) acc += el.children.length;
    return acc;
  }, 0);
};
...
const extractStat = (statName, arr) => {
  // Solution code here...
  let theSpecificOne = arr.reduce((acc, el) => {
    if (el.stat.name === statName) {
      acc = el;
    }
    return acc;
  }, {});
  if (theSpecificOne.stat) return theSpecificOne;
  else return null;
};
...
const extractChildren = (arr) => {
  ...
  const filteredArr = arr.filter((el) => {
    return el.name.includes("a");
  });
  return filteredArr.reduce((acc, el) => {
    if (el.children)
