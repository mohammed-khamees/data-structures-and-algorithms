...
const standardizePhoneNumbers = (arr) => {
  ...
  let numbers = arr.map((num) => {
    ...
    return numArr
      .filter((num) => {
        return Number(num) || (Number(num) === 0 && num !== " ");
      })
      .join("");
  });
...
const unenrollBrook = (arr) => {
  // Solution code here...
  let newArr = arr.reduce((acc, array) => {
    acc.push(array.filter((el) => !el.includes("Brook")));
    return acc;
  }, []);
